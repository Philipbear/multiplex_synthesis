...
def process_file(file, result_dir):
    ...
    # contain human matches
    df_human = df_grouped[df_grouped['ncbi_ids'].apply(lambda x: '9606' in x)].reset_index(drop=True)
    human_usis = df_human['lib_usi'].unique().tolist()
    # df_human = df[df['lib_usi'].isin(human_usis)].reset_index(drop=True)

    return len(human_usis)
...
def perform_analysis_main(processed_output_path, out_dir):
    ...
    all_human_usis_count = sum(results)
    print(f"Total unique human lib_usis across all public files: {all_human_usis_count}")
