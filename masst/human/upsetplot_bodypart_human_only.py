...
def create_upset_plot_by_usi(tsv_path, output_path, figsize=(8.6, 2.15), min_subset_size=10):
    ...
    # print some stats
    # how many structures have matches only in UBERONBodyPartName 'feces'
    feces_only_count = binary_df[binary_df.sum(axis=1) == 1]['feces'].sum()
    print(f"Structures with matches only in 'feces': {feces_only_count}")
    urine_only_count = binary_df[binary_df.sum(axis=1) == 1]['urine'].sum()
    print(f"Structures with matches only in 'urine': {urine_only_count}")
    ...
    upset = UpSet(
        ...
        element_size=3,
        facecolor='0.5',
        connecting_line_width=0.85  # adjusted `upsetplot` package, add this parameter
    )
