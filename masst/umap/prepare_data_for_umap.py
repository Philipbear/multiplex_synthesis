...
def load_and_preprocess_masst_data(base_dir_path, min_matches_per_usi=3):
    """
    Load and preprocess MASST data once for all subsequent processing
    """
    
    files = [f for f in os.listdir(base_dir_path) if f.endswith('.pkl')]
    print(f"Processing {len(files)} files using {n_cores} cores...")
    
    files = [os.path.join(base_dir_path, f) for f in files]
    
    pool = mp.Pool(n_cores)
    process_func = partial(preprocess_file, min_matches_per_usi=min_matches_per_usi)
    results = list(tqdm(pool.imap(process_func, files), total=len(files), desc="Processing MASST files"))
    pool.close()
    pool.join()
    
    # More efficient concatenation
    all_df = pd.concat(results, ignore_index=True, copy=False)
    
    return all_df


def preprocess_file(file_path, min_matches_per_usi=3):
    """
    Load and preprocess MASST data once for all subsequent processing
    """
...
def process_all_ranks(base_dir_path, base_umap_dir, min_matches_per_usi=3):
    """
    Main function to process all taxonomic ranks efficiently
    """
    print("="*80)
    print("MASST DATA PROCESSING FOR ALL RANKS")
    
    # Load and preprocess data once
    df = load_and_preprocess_masst_data(base_dir_path, min_matches_per_usi)
...
if __name__ == '__main__':
    ...
    base_dir_path = '/home/shipei/projects/synlib/masst/processed_output'
