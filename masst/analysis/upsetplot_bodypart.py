...
def create_upset_plot_by_usi(tsv_path, output_path, figsize=(8.6, 2.15), min_subset_size=10):
    ...
    for ax_name, ax in axes_dict.items():
        if ax_name == 'intersections':
            ax.tick_params(axis='both', which='major', length=1, width=0.5, labelsize=3.5, color='0.3', pad=1)
...
def create_upset_plot_by_structure(tsv_path, output_path, figsize=(8.6, 2.15), min_subset_size=10):
    """
    Create UpSet plot directly from TSV file containing bodypart data based on unique structures
    
    Args:
        tsv_path: Path to TSV file with columns ['inchikey_2d', 'UBERONBodyPartName', 'count']
        output_path: Path to save the SVG plot
        figsize: Figure size tuple
        min_subset_size: Minimum subset size to show in plot
    """
...
if __name__ == '__main__':
    ############# human
    create_upset_plot_by_structure(
        tsv_path="masst/analysis/data/bodypart/human_compound_bodypart_counts.tsv",
        output_path="masst/analysis/plots/upset_human_bodypart_structures.svg",
        figsize=(8.6, 2.5),
        min_subset_size=10
    )
    
    create_upset_plot_by_usi(
        tsv_path="masst/analysis/data/bodypart/human_bodypart_usi_raw_counts.tsv",
        output_path="masst/analysis/plots/upset_human_bodypart_USIs.svg",
        figsize=(8.6, 2.75),
        min_subset_size=15
    )
    ...
    ############# rodent
    create_upset_plot_by_structure(
        tsv_path="masst/analysis/data/bodypart/rodent_compound_bodypart_counts.tsv",
        output_path="masst/analysis/plots/upset_rodent_bodypart_structures.svg",
        figsize=(8.6, 2.5),
        min_subset_size=10
    )
    
    create_upset_plot_by_usi(
        tsv_path="masst/analysis/data/bodypart/rodent_bodypart_usi_raw_counts.tsv",
        output_path="masst/analysis/plots/upset_rodent_bodypart_USIs.svg",
        figsize=(8.6, 2.75),
        min_subset_size=15
    )