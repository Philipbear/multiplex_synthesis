...
def load_data():
    ...
    # Create combined exposure categories for coloring
...
def create_boxplot(df, output_path):
    """
    Create box plot for log intensity values grouped by current 5-ASA use and colored by exposure category.
    
    Args:
        df (pd.DataFrame): DataFrame with columns 'intensity', '5ASA_exposure_category', 'current_use_group'
    ...
    use_groups = ['No', 'Yes']  # Groups based on current 5-ASA use
    
    for i, use_group in enumerate(use_groups):
        group_data = df[df['current_use_group'] == use_group]['log_intensity']
        n_samples = len(group_data)
        box_data.append(group_data)
        box_label_text = f'{use_group}\n(n = {n_samples})'
        box_labels.append(box_label_text)
    ...
    # Customize the plot
    ax.set_xlabel('5-ASA current use', fontsize=6, color='0.2', labelpad=2)
    ax.set_ylabel('5-ASA-phenylpropionate\npeak area (log10)', fontsize=5.5, color='0.2')
    ax.set_title('Adult IBD cohort', fontsize=7, color='0.2', pad=30)
...
if __name__ == "__main__":
    ...
    create_boxplot(df, 'target_drugs/5-ASA-PP/datasets/plots/boxplot_2.svg')