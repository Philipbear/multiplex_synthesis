...
def plot_disease_heatmap(input_data, output_path):
    ...
    df['name'] = df['name'].apply(lambda x: x.replace('2-(4-isobutylphenyl)propionic acid (Ibuprofen)', 'Ibuprofen').replace('1-hydroxyIbuprofen', '1-Hydroxyibuprofen'))  # specific name replacement
...
if __name__ == '__main__':
    ...
    # Prepare the data
    masst_pkl_path = 'target_drugs/ibuprofen_conjugates/data/ibuprofen_masst_matches.pkl'
    compound_disease_data = prepare_data(masst_pkl_path)
    
    # Save the processed data
    compound_disease_data.to_csv('target_drugs/ibuprofen_conjugates/data/compound_disease_data.tsv', sep='\t', index=False)

    # Generate and save the heatmap
    plot_disease_heatmap(
        compound_disease_data,
        'target_drugs/ibuprofen_conjugates/plots/disease_heatmap.svg'
    )